...
import { MapPin, ArrowRight, Navigation, Heart } from 'lucide-react';

const MustVisitPage = () => {
  const { t } = useTranslation();
  const navigate = useNavigate();

  // LocalStorage ကနေ wishlist ကို ယူမယ်
  const [wishlist, setWishlist] = useState(() => {
    return JSON.parse(localStorage.getItem('user_wishlist') || '[]');
  });

  // --- FIXED TOGGLE WISHLIST FUNCTION ---
  const toggleWishlist = (e, item, type) => {
    e.stopPropagation(); // Card click event ကို တားတာ
    let updatedWishlist = JSON.parse(localStorage.getItem('user_wishlist') || '[]');
    const isExist = updatedWishlist.find(i => i.id === item.id);

    if (isExist) {
      updatedWishlist = updatedWishlist.filter(i => i.id !== item.id);
    } else {
      // Profile Page နဲ့ ကိုက်အောင် data format ကို ညှိသိမ်းမယ်
      updatedWishlist.push({
        id: item.id,
        title: item.titleKey, 
        image: type === 'experience' ? item.img : item.image, // Image key ကွဲတာကို handle လုပ်တာ
        type: type, // 'experience' သို့မဟုတ် 'attraction'
        location: 'Kurashiki',
        price: type === 'experience' ? '' : item.fee
      });
    }
    ...
    <div className="bg-[#fcfdfa] min-h-screen">
      ...
      <section className="relative min-h-[95vh] flex items-center justify-center px-6 bg-[#072d07] text-white overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            ...
            >
              ...
              <div className="max-w-md">
                ...
                <motion.button
                  onClick={() => document.getElementById('spots-section')?.scrollIntoView({ behavior: 'smooth' })}
            ...
            >
              <div className="relative aspect-[4/5] w-[420px] ml-auto rounded-[4rem] overflow-hidden shadow-[0_40px_80px_-15px_rgba(0,0,0,0.5)] border-[1px] border-white/10">
                <img src="./images/whiteHouse19.jpg" alt="Nature" className="w-full h-full object-cover brightness-90" />
        ...
        <div className="absolute bottom-0 left-0 w-full overflow-hidden leading-[0] translate-y-[2px]">
          <svg viewBox="0 0 1440 320" className="relative block w-full h-[120px] md:h-[200px]" preserveAspectRatio="none">
            <path fill="#fcfdfa" d="M0,160L80,176C160,192,320,224,480,213.3C640,203,800,149,960,138.7C1120,128,1280,160,1360,176L1440,192L1440,320L1360,320C1280,320,1120,320,960,320C800,320,640,320,480,320C320,320,160,320,80,320L0,320Z"></path>
          </svg>
        </div>
      </section>
      ...
      <section id="spots-section" className="py-16 px-6 max-w-7xl mx-auto">
        <div className="flex flex-col bg-green-800 p-8 rounded-[2.5rem] md:flex-row justify-between items-center mb-12 gap-6 shadow-xl border border-green-700/50 mt-10">
          <div>
            <h2 className="text-3xl text-stone-100 italic font-serif mb-1">{t('mustVisit.spots_title')}</h2>
            <span className="text-stone-300 font-bold text-[9px] uppercase tracking-[0.3em]">{t('mustVisit.spots_subtitle')}</span>
          </div>
            ...
            {filteredSpots.map((spot) => (
              ...
              >
                <div className="relative aspect-4/3 overflow-hidden">
                  ...
                  {/* Fixed Heart Button for Spots */}
                  <button
                    onClick={(e) => toggleWishlist(e, spot, 'attraction')}
                    className="absolute top-5 right-5 z-20 p-3 rounded-full backdrop-blur-md transition-all duration-300"
                    style={{ backgroundColor: wishlist.find(i => i.id === spot.id) ? '#ef4444' : 'rgba(255,255,255,0.7)' }}
                  >
                    <Heart size={16} fill={wishlist.find(i => i.id === spot.id) ? "white" : "none"} color={wishlist.find(i => i.id === spot.id) ? "white" : "#444"} />
                  </button>
                  
                  <div className="absolute top-5 left-5">
                    <span className="bg-white/90 backdrop-blur-md text-stone-900 px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-wider">
                ...
                <div className="p-6">
                  <div className="flex justify-between items-start mb-1">
                    <h3 className="font-serif italic text-stone-900 group-hover:text-green-800 transition-colors text-xl">{t(spot.titleKey)}</h3>
                    <a href={spot.mapUrl} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="w-10 h-10 rounded-full bg-stone-50 flex items-center justify-center text-stone-400 hover:bg-green-100 hover:text-green-700">
                      <MapPin size={18} />
                    </a>
                  </div>
                  <p className="text-stone-500 text-sm leading-relaxed mb-3 line-clamp-2 italic font-light">{t(spot.descKey)}</p>
                  <div className="flex items-center gap-6 mb-1 text-[10px] font-bold text-stone-400 border-t border-stone-50 pt-4">
      ...
      <section id="experiences" className="py-24 px-6 max-w-7xl mx-auto">
        <div className="flex flex-col bg-green-800 p-8 rounded-[2.5rem] md:flex-row justify-between items-center mb-12 gap-6 shadow-xl border border-green-700/50">
          <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {tabs.map(tab => (
              <button
                ...
                className={`px-5 py-1.5 rounded-full text-[8px] font-black uppercase tracking-widest transition-all border ${activeExpTab === tab.id ? 'bg-stone-900 border-stone-900 text-white shadow-lg' : 'bg-transparent border-stone-200/30 text-stone-100 hover:border-white'}`}
              >
                {tab.label}
              </button>
            ))}
        ...
        <motion.div layout className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          <AnimatePresence mode='popLayout'>
            {filteredExperiences.slice(0, 40).map((exp) => (
              <motion.div layout key={exp.id} whileHover={{ y: -8 }} className="group cursor-pointer" onClick={() => navigate(`/experience/${exp.id}`)}>
                <div className="relative aspect-3/4 md:aspect-4/5 rounded-[2rem] overflow-hidden mb-4 shadow-sm group-hover:shadow-2xl transition-all duration-500">
                  ...
                  {/* FIXED: Heart Button for Experiences (Using 'exp' instead of 'spot') */}
                  <button
                    onClick={(e) => toggleWishlist(e, exp, 'experience')}
                    className="absolute top-5 right-5 z-30 p-3 rounded-full backdrop-blur-md transition-all duration-300"
                    style={{ backgroundColor: wishlist.find(i => i.id === exp.id) ? '#ef4444' : 'rgba(255,255,255,0.7)' }}
                  >
                    <Heart size={16} fill={wishlist.find(i => i.id === exp.id) ? "white" : "none"} color={wishlist.find(i => i.id === exp.id) ? "white" : "#444"} />
                  </button>
                  ...
                  <div className="absolute bottom-6 left-6 right-6 text-white">
                    ...
                    <div className="flex gap-2 opacity-80">
                      {exp.tags.map(tag => (
                        <span key={tag} className="text-[8px] text-green-300 uppercase font-bold tracking-wider">
